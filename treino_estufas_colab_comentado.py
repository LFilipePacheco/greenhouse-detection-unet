...
EXPERIMENTO = 'E1'                     # 'E0', 'E1', 'E2'... (usado nos nomes dos ficheiros)
ARQUITETURA = 'unet'                   # 'unet' | 'efficientnetb0' | 'resnet34'
TVERSKY_ALPHA = 0.3                    # penalização de falsos negativos
TVERSKY_BETA = 0.7                     # penalização de FALSOS POSITIVOS (E0: 0.5)
TVERSKY_GAMMA = 1.33
USAR_NEGATIVOS_ESTRADAS = True         # hard negatives centrados em estradas (E0: False)

# Ficheiros de entrada (recortes — NUNCA o raster de 19 GB completo)
TIFF_TREINO   = DATA + 'ortoSat_treino.tif'
SHP_ESTUFAS   = DATA + 'estufas_treino.shp'        # estufas digitalizadas (treino)
SHP_MASCARA   = DATA + 'mascara_treino.shp'        # área de treino
SHP_ESTRADAS  = DATA + 'estradas_buffer.shp'       # buffer OSM (QuickOSM)

# Teste / avaliação
TIFF_TESTE    = DATA + 'ortoSat_teste.tif'
SHP_MASC_TESTE= DATA + 'mascara_teste.shp'
SHP_VERDADE   = DATA + 'verdade_terreno_teste.shp' # estufas reais na máscara de teste
...
EPOCHS = 120
LEARNING_RATE = 5e-4
N_AUGMENT = 2                          # augmentações por patch (2 => dataset x3)
MIN_ESTUFA_PIXELS = 100
FRACAO_NEGATIVOS = 0.33                # negativos = 33% dos positivos
SEED = 42

# Pós-processamento da inferência
CONFIDENCE_THRESHOLD = None            # None = usa o melhor threshold encontrado na validação
MIN_AREA_M2, MAX_AREA_M2 = 40, 3000
MIN_COMPACIDADE, MAX_ALONGAMENTO = 0.25, 4.0
