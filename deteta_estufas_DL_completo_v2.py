...
# === CONFIGURAÇÃO ===
RASTER_PATH = r"data/ortofoto_zona_vulneravel.tif"
MODEL_PATH = r"models/modelo_melhorado_20250821_164822.h5"
OUTPUT_DIR = r"output"

# Área de teste
USE_MASK = True
MASK_PATH = r"data/deteta_mascara_1.shp"
...
# Filtro geométrico anti-estradas (E4 do plano de ação)
FILTRO_GEOMETRICO = True
MIN_COMPACIDADE = 0.25   # 4*pi*A/P^2 (círculo=1; estradas têm valores muito baixos)
MAX_ALONGAMENTO = 4.0    # razão lado maior/lado menor do retângulo mínimo
...
def normalize_image(image):
    """
    Normaliza imagem para [0, 1] — CORREÇÃO CRÍTICA:
    O treino usou X/255.0. A versão anterior deste script usava uma
    normalização por percentis (p2-p98), incompatível com o treino,
    o que fazia o modelo classificar campos inteiros como estufas.
    A normalização de inferência TEM de ser idêntica à do treino.
    """
...
model = keras.models.load_model(
    rr"models/unet_estufas_improved.h5",
    compile=False
)

# Salvar sem optimizer
model.save(
    rr"models/modelo_limpo.h5",
...
if USE_MASK and os.path.exists(MASK_PATH):
    ...
    with rasterio.open(RASTER_PATH) as src:
        ...
        # CORREÇÃO: src.index(x, y) devolve (row, col) — a versão antiga
        # desempacotava como (col, row), transpondo a janela!
        row_min, col_min = src.index(bounds[0], bounds[3])  # canto sup. esquerdo
        row_max, col_max = src.index(bounds[2], bounds[1])  # canto inf. direito
        ...
        print(f"  Janela (bbox da máscara): {TEST_AREA.width}x{TEST_AREA.height} pixels")
        
        area_poligono_ha = mask_gdf.geometry.area.sum() / 10000
        print(f"  Área real do polígono da máscara: {area_poligono_ha:.0f} ha")
else:
...
with rasterio.open(RASTER_PATH) as src:
    image = src.read([1, 2, 3], window=TEST_AREA)  # apenas RGB, descarta o canal alpha
    ...
    print(f"  Área da janela retangular processada: ~{area_ha:.0f} ha (a predição será depois recortada ao polígono)")
    ...
    for i in range(0, max(1, height - PATCH_SIZE + 1), stride):
        for j in range(0, max(1, width - PATCH_SIZE + 1), stride):
            ...
            if patch.shape[0] < PATCH_SIZE or patch.shape[1] < PATCH_SIZE:
                padded = np.zeros((PATCH_SIZE, PATCH_SIZE, image_norm.shape[2]), dtype=np.float32)
...
# === RECORTAR AO POLÍGONO DA MÁSCARA (CORREÇÃO) ===
# A janela processada é o retângulo envolvente; aqui zeramos tudo o que
# está fora do polígono real da máscara, para não haver deteções fora dela.
if USE_MASK and os.path.exists(MASK_PATH):
    mascara_poligono = rasterize(
    ...
    dentro = int(mascara_poligono.sum())
    print(f"  Recorte ao polígono: {dentro:,} px dentro "
          f"({100 * dentro / (height * width):.0f}% da janela)")
    prediction = prediction * mascara_poligono
...
def metricas_forma(poly):
    """Compacidade (círculo=1) e alongamento (retângulo mínimo rodado)."""
    compacidade = 4 * np.pi * poly.area / max(poly.length ** 2, 1e-6)
    rect = poly.minimum_rotated_rectangle
    try:
        cs = list(rect.exterior.coords)
        lados = [np.hypot(cs[k+1][0]-cs[k][0], cs[k+1][1]-cs[k][1]) for k in range(4)]
        alongamento = max(lados[0], lados[1]) / max(min(lados[0], lados[1]), 0.1)
    except Exception:
        alongamento = 1.0
    return compacidade, alongamento

polygons = []
atributos = []   # (compacidade, alongamento) por polígono aceite
rejected = {'small': 0, 'large': 0, 'road_like': 0}
        ...
        if len(coords) >= 3:
            try:
                ...
                if area < MIN_AREA_M2:
                    ...
                    compac, along = metricas_forma(poly)
                    # Filtro anti-estradas: formas muito alongadas ou muito
                    # pouco compactas são troços de estrada, não estufas
                    if FILTRO_GEOMETRICO and (compac < MIN_COMPACIDADE or along > MAX_ALONGAMENTO):
                        rejected['road_like'] += 1
                    else:
                        polygons.append(poly)
                        atributos.append((compac, along))
            except:
                continue
...
if rejected['road_like'] > 0:
    print(f"  {rejected['road_like']} rejeitadas por forma de estrada "
          f"(compacidade<{MIN_COMPACIDADE} ou alongamento>{MAX_ALONGAMENTO})")
...
Forma estrada: {rejected['road_like']}
...
if len(polygons) > 0:
    ...
    gdf = gpd.GeoDataFrame({
        ...
        'compac': [a[0] for a in atributos],
        'along': [a[1] for a in atributos],
        'geometry': polygons
    }, crs=crs)
